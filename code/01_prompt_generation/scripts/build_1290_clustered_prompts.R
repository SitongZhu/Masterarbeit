...
#   - outcome mapping: values_Links / values_Neutral / values_Rechts (character codes)
...
columns_wave_suffix <- c("010","020","1130","1290", "1500","780","2320","011","060","820")

# Which waves to use (numbers only)
waves_to_use <- c(10, 11, 14, 15, 22, 23, 25, 26)

# Outcome variable suffix (column names kp{n}_{suffix} within each wave)
outcome_suffix <- "1290"

# outcome_question (set manually)
outcome_question <- "Manche meinen, dass die Bekämpfung des Klimawandels auf jeden Fall Vorrang
haben sollte, auch wenn das dem Wirtschaftswachstum schadet. Andere meinen, dass das
Wirtschaftswachstum auf jeden Fall Vorrang haben sollte, auch wenn das die Bekämpfung des
Klimawandels erschwert. Wie ist Ihre Meinung zu diesem Thema?"

# Outcome groups: data.frame with values_* columns; suffix after values_ becomes output label
# Each column is a list-column (I(list(...))) of raw Stata codes (character)
outcome_groups <- data.frame(
  values_Vorrang_fuer_Bekaempfung_des_Klimawandels   = I(list(as.character(1:3))),    # left
  values_Mittelposition = I(list(as.character(4))),  # neutral
  values_Vorrang_fuer_Wirtschaftswachstum  = I(list(as.character(5:7)))  # right
)
...
# Named with outcome_suffix only: wave_list_for_llm_join_1290.rds
wave_list_export_path <- file.path(hdata_dir, paste0("wave_list_for_llm_join_", outcome_suffix, ".rds"))
# Per-wave column dictionary: harmonized names (e.g. 020), var labels, value-label JSON
wave_column_metadata_export_path <- file.path(hdata_dir, paste0("wave_column_metadata_", outcome_suffix, ".rds"))

# Outcome codes treated as missing/invalid before trichotomy
values_to_exclude <- c(
...
# Outcome option labels: from outcome_groups column names (values_ prefix stripped)
OUTCOME_OPTIONS_DE <- sub("^values_", "", names(outcome_groups))
if (length(OUTCOME_OPTIONS_DE) < 2L) stop("outcome_groups needs at least two values_* columns.")
...
recode_outcome_to_factor <- function(oc) {
  oc <- as.character(oc)
  k <- length(OUTCOME_OPTIONS_DE)
  out <- rep(NA_integer_, length(oc))
  for (i in seq_len(k)) {
    col_nm <- paste0("values_", OUTCOME_OPTIONS_DE[i])
    if (!col_nm %in% names(outcome_groups)) next
    vals <- outcome_groups[[col_nm]][[1L]]
    out[oc %in% as.character(vals)] <- i
  }
  factor(out, levels = seq_len(k), labels = OUTCOME_OPTIONS_DE)
}
...
for (nm in selected) {
  ...
  df[["outcome"]] <- df[[col_name]]
  oc <- as.character(df[["outcome"]])
  df <- df[!(oc %in% exclude_char), , drop = FALSE]
  # Raw code + Stata value label (sublabel) for RDS/joins; not written into prompt JSON
  df[["outcome_code"]] <- trimws(as.character(haven::zap_labels(df[[col_name]])))
  attr(df, "outcome_source_column") <- col_name
  df[["outcome"]] <- recode_outcome_to_factor(df[[col_name]])
...
# Outcome-Wert auf Label aus OUTCOME_OPTIONS_DE abbilden (Outcome ist bereits bereinigt)
map_outcome_to_label <- function(value) {
  if (is.na(value)) return(OUTCOME_OPTIONS_DE[1L])  # Fallback: erste Option
  if (is.factor(value)) return(as.character(value))
  lbl <- get_value_label(value)
  if (!is.na(lbl) && nzchar(trimws(lbl)) && lbl != "Nicht gefragt / keine Angabe") return(lbl)
  if (length(OUTCOME_OPTIONS_DE) >= 4L) return(OUTCOME_OPTIONS_DE[(length(OUTCOME_OPTIONS_DE) %/% 2L) + 1L])  # Mitte als Fallback
  OUTCOME_OPTIONS_DE[1L]
}
...
build_instruction <- function(field_start, field_end, background_text, question, choices_text, temporal_context = NULL) {
  ...
  glue(
    ...
    "\"{question}\"\n\n",
    "Antwortformat: Wählen Sie genau eine der folgenden nummerierten Optionen.\n",
    "{choices_text}\n\n",
    "Geben Sie als Ihre **gesamte** Antwort ausschließlich den Klartext aus, der in der gewählten Zeile **nach** der Nummer und dem Punkt steht (also ohne die Ziffer, ohne den Punkt und ohne Anführungszeichen).\n",
    "WICHTIG: Kein JSON, keine Erläuterung und kein Text vor oder nach dem Kategorienamen."
  )
}
...
choices_text <- paste(seq_along(OUTCOME_OPTIONS_DE), OUTCOME_OPTIONS_DE, sep = ". ", collapse = "\n")
...
build_instruction_trajectory <- function(year_prev, response_prev, field_start_prev, field_end_prev,
  ...
  block_vorwelle <- glue(
    "**Antwort in der Vorwelle:** Im Jahr {y_prev} (in der zuvor durchgeführten Befragung{prev_survey_extra}) haben Sie darauf geantwortet: \"{response_prev}\".\n\n---\n\n"
  )
  part1 <- glue(
    "Sie sind ein*e Sozialwissenschaftler*in und analysieren Befragungsdaten einer befragten Person.\n\n",
    "**Zeitlicher Geltungsbereich der Daten (verbindlich):**\n",
...
for (nm in names(wave_list)) {
  ...
  for (i in seq_len(nrow(df))) {
    ...
    target_answer <- map_outcome_to_label(row[["outcome"]])
    ...
    if (prev_nm %in% names(wave_list)) {
      ...
      if (nrow(df_traj) > 0L) {
        ...
        for (j in seq_len(nrow(df_traj))) {
          ...
          response_prev <- map_outcome_to_label(prev_row[["outcome"]])
          fs <- row[["field_start"]]
          fe <- row[["field_end"]]
