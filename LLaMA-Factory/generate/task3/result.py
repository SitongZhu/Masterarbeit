...
def analyze_single_prediction(prediction_str: str, label_str: str) -> dict:
    ...
    # prediction_reason = prediction_json.get("reason", "")  # 注释掉：不需要reason分析
    ...
    # label_reason = label_json.get('reason', "")  # 注释掉：不需要reason分析
    ...
    return {
        ...
        # 'prediction_reason': prediction_reason,  # 注释掉：不需要reason分析
        'label': label,
        # 'label_reason': label_reason,  # 注释掉：不需要reason分析
        'is_correct': is_correct,
        # 'similarities': similarities  # 注释掉：不需要相似度分析
    }
...
def analyze_predictions_file(file_path: str) -> dict:
    ...
    categories = defaultdict(lambda: {
        ...
        # 'vocab_overlap': 0, 'jaccard_sim': 0, 'cosine_sim': 0,  # 已注释掉：不需要相似度分析
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, 1):
            try:
                ...
                # categories[label]['vocab_overlap'] += similarities['vocabulary_overlap']  # 已注释掉
                # categories[label]['jaccard_sim'] += similarities['jaccard_similarity']  # 已注释掉
                # categories[label]['cosine_sim'] += similarities['cosine_similarity']  # 已注释掉
                ...
                if analysis['is_correct']:
                    ...
                    categories[label]['error_details'].append({
                        ...
                        # 'prediction_reason': analysis['prediction_reason'],  # 已注释掉：不需要reason分析
                        # 'label_reason': analysis['label_reason']  # 已注释掉：不需要reason分析
                    })
    ...
    return {
        ...
        # 'avg_vocab_overlap': avg_vocab_overlap,  # 已注释掉：不需要相似度分析
        # 'avg_jaccard_sim': avg_jaccard_sim,  # 已注释掉：不需要相似度分析
        # 'avg_cosine_sim': avg_cosine_sim,  # 已注释掉：不需要相似度分析
...
def analyze_file_info(filename):
    ...
    if 'Distill_Qwen-14B' in filename:
        ...
        info['version'] = '-'
    elif 'Qwen2.5-7B' in filename or 'Qwen2_5-7B' in filename:
    ...
    with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
        ...
        for result in all_results:
            overview_data.append([
                ...
                # f"{result['avg_vocab_overlap']:.2f}",  # 已注释掉：不需要相似度分析
                # f"{result['avg_jaccard_sim']:.4f}",  # 已注释掉：不需要相似度分析
                # f"{result['avg_cosine_sim']:.4f}"  # 已注释掉：不需要相似度分析
            ])

        overview_df = pd.DataFrame(overview_data, columns=[
            '文件名', '模型', '版本', '文件类型', '总样本数', '正确预测数', '准确率',
            '微平均精确率', '微平均召回率', '微平均F1'
            # '平均词汇重叠', 'Jaccard相似度', '余弦相似度'  # 已注释掉：不需要相似度分析
        ])
            ...
            for category in all_categories:
                if category in result['category_metrics']:
                    ...
                    row_data.extend([
                        ...
                        # f"{metrics['avg_vocab_overlap']:.2f}",  # 已注释掉：不需要相似度分析
                        # f"{metrics['avg_jaccard_sim']:.4f}",  # 已注释掉：不需要相似度分析
                        # f"{metrics['avg_cosine_sim']:.4f}"  # 已注释掉：不需要相似度分析
                    ])
                else:
                    # 如果该文件没有这个类别的数据，填入N/A
                    row_data.extend(['N/A'])  # 已注释掉相似度相关列
        ...
        for category in all_categories:
            columns.extend([
                f'{category}_准确率'
                # f'{category}_词汇重叠',  # 已注释掉：不需要相似度分析
                # f'{category}_Jaccard相似度',  # 已注释掉：不需要相似度分析
                # f'{category}_余弦相似度'  # 已注释掉：不需要相似度分析
            ])
                ...
                if 'mismatches' in result and result['mismatches']:
                    for mismatch in result['mismatches']:
                        mismatch_data.append([
                            ...
                            mismatch['prediction'],
                            mismatch['label']
                            # mismatch.get('prediction_reason', '')[:100] + "..." if len(  # 已注释掉：不需要reason分析
                            #     mismatch.get('prediction_reason', '')) > 100 else mismatch.get('prediction_reason', ''),
                            # mismatch.get('label_reason', '')[:100] + "..." if len(  # 已注释掉：不需要reason分析
